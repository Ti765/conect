...
function SubmitButton() {
  ...
  return (
    <Button
      ...
      disabled={pending}
      size="lg"
      className="w-full sm:w-auto bg-gradient-to-r from-primary via-blue-600 to-purple-600 hover:opacity-90 text-primary-foreground"
    >
      {pending ? (
        <>
...
export default function SeparateCtePage() {
  ...
  const [formKey, setFormKey] = useState(Date.now()); // reinicia o formulário (inclui input de arquivo)

  const handleReset = () => {
    setFormKey(Date.now());
  };

  // Faz download do .zip gerado no backend
  const handleDownload = (zipBase64: string, zipName: string) => {
  ...
  useEffect(() => {
    if (state.message && !state.success) {
      // aqui você pode exibir toast de erro, se desejar
    }
  }, [state]);
        ...
        <form action={formAction} key={formKey}>
          ...
          <CardContent className="space-y-6">
            <div className="grid w-full max-w-lg items-center gap-2">
              <Label htmlFor="cte-files" className="sr-only">
                Arquivos CT-e
              </Label>
              <Input
                id="cte-files"
            ...
            {/* Área de drag‑and‑drop apenas visual */}
        ...
        <Card className="shadow-lg mt-8">
          <CardHeader>
            ...
            <CardDescription>
              {state.success
                ? state.message
                : "Processamento concluído com os seguintes resultados e/ou erros."}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <Accordion
              type="multiple"
              defaultValue={["simples", "normal", "zerado", "erros"]}
            >
                ...
                <AccordionItem value="simples">
                  <AccordionTrigger>
                    <div className="flex items-center">
                      ...
                      Simples Nacional (
                      {state.analysisResult.simplesNacional.length})
                    </div>
                  </AccordionTrigger>
                  <AccordionContent>
                    <ScrollArea className="h-40 w-full rounded-md border p-2">
                      <ul className="list-disc pl-5 text-sm">
                        {state.analysisResult.simplesNacional.map((file, idx) => (
                          <li key={`sn-${idx}`}>{file.name}</li>
                        ))}
                      </ul>
                    </ScrollArea>
                  </AccordionContent>
                ...
                <AccordionItem value="normal">
                  <AccordionTrigger>
                    <div className="flex items-center">
                      ...
                      Regime Normal (
                      {state.analysisResult.regimeNormal.length})
                    </div>
                  </AccordionTrigger>
                  <AccordionContent>
                    <ScrollArea className="h-40 w-full rounded-md border p-2">
                      <ul className="list-disc pl-5 text-sm">
                        {state.analysisResult.regimeNormal.map((file, idx) => (
                          <li key={`rn-${idx}`}>{file.name}</li>
                        ))}
                      </ul>
                    </ScrollArea>
                  </AccordionContent>
                ...
                <AccordionItem value="zerado">
                  <AccordionTrigger>
                    <div className="flex items-center">
                      ...
                      CT-es com Valor Zerado (
                      {state.analysisResult.valorZerado.length})
                    </div>
                  </AccordionTrigger>
                  <AccordionContent>
                    <ScrollArea className="h-40 w-full rounded-md border p-2">
                      <ul className="list-disc pl-5 text-sm">
                        {state.analysisResult.valorZerado.map((file, idx) => (
                          <li key={`vz-${idx}`}>{file.name}</li>
                        ))}
                      </ul>
                    </ScrollArea>
                  </AccordionContent>
            ...
            {/* Botão de download aparece apenas se o backend retornou o ZIP */}
            {state.zipBase64 && state.zipName && (
              <Button
                className="mt-4"
                variant="default"
                onClick={() =>
                  handleDownload(state.zipBase64 as string, state.zipName as string)
                }
              >
                <Download className="mr-2 h-5 w-5" />
                Baixar Arquivos Classificados (.zip)
              </Button>
            )}
              ...
              state.analysisResult.errors.length === 0 && (
                <Alert className="bg-green-500/10 border-green-500/50 text-green-700 dark:text-green-400">
                  ...
                  <AlertTitle className="font-semibold">
                    Nenhum arquivo processado ou encontrado
                  </AlertTitle>
                  <AlertDescription>
